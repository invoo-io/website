...
export function FeaturedArticleCard({ post, locale }: FeaturedArticleCardProps) {
  return (
    ...
    >
      <article className="grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-8 min-h-[400px] md:min-h-[500px]">
        {/* Left side: Image (3/4 columns) */}
        <div className="lg:col-span-3 relative rounded-[24px] overflow-hidden bg-bg-secondary">
          {post.coverImage ? (
            <Image
              src={post.coverImage}
              alt={post.title}
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-accent-blue-main/20 to-accent-purple-main/20 flex items-center justify-center">
              <span className="text-6xl md:text-8xl font-bold text-label-tertiary/20">
                {post.title.charAt(0)}
              </span>
            </div>
          )}
        ...
        <div className="lg:col-span-1 flex flex-col justify-between py-4">
          <div>
            ...
            {/* Excerpt */}
            <p className="text-body text-label-secondary mb-6 line-clamp-3">
              {post.excerpt}
            </p>
          </div>
